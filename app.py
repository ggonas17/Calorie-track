...
def library():
    ...
    for f in favs:
        ...
        <div class="log-item fav-search-item" style="cursor:pointer; display:flex;" onclick="window.location.href='/edit_fav/{f['id']}'" data-name="{f['food_name'].lower()}">
            ...
            <button style="background:#0a84ff; color:#fff; font-weight:bold; font-size:1.5rem; padding:5px 15px; border-radius:12px; margin-left:10px; border:none; cursor:pointer;" onclick="event.stopPropagation(); quickAddPrompt('{safe_name}', '{f['qty'] or 1}', '{f['unit'] or 'qty'}', {f['calories']}, {f['protein']}, '{recipe_safe}')">+</button>
        </div>"""
...
def edit_fav(fav_id):
    ...
    if request.method == 'POST':
        ...
        conn.execute('UPDATE favorites SET food_name=?, qty=?, unit=?, calories=?, protein=?, recipe=? WHERE id=?', (f_name, float(q_val), u_val, int(float(c_val)), int(float(p_val)), r_val, fav_id))
...
def edit_log(log_id):
    ...
    if request.method == 'POST':
        ...
        conn.execute('UPDATE logs SET food_name=?, qty=?, unit=?, calories=?, protein=?, recipe=? WHERE id=?', (f_name, float(q_val), u_val, int(float(c_val)), int(float(p_val)), r_val, log_id))
...
def build_meal():
    ...
    if request.method == 'POST':
        ...
        conn.execute('INSERT INTO logs (food_name, qty, unit, calories, protein, timestamp, date, recipe) VALUES (?, 1, "qty", ?, ?, ?, ?, ?)', (m_name, int(float(m_cal)), int(float(m_prot)), now_time, today, m_recipe))
        if save_lib: save_fav_db(conn, m_name, 1, "qty", int(float(m_cal)), int(float(m_prot)), m_recipe)
    ...
    for f in favs:
        safe_name = f['food_name'].replace('"', '&quot;').replace("'", "\\'")
        recipe_safe = (f['recipe'] or "").replace('"', '&quot;')
        html_sugs += f"""<div onclick="let el=this; let q=prompt('How much did you eat?\\n{safe_name} (Saved as {f['qty'] or 1} {f['unit'] or 'qty'})', '{f['qty'] or 1}'); if(q!==null && q.trim()!==''){{ q=parseFloat(q.replace(',','.')); if(!isNaN(q) && q>0){{ let bq=parseFloat('{f['qty'] or 1}'); let multi=q/bq; let recipe='{recipe_safe}'; if(recipe && recipe!=='\\'\\"\\"\\' && recipe!=='\\[\\]'){{ let p=JSON.parse(recipe); p=p.map(i=>{{ return{{...i, qty: i.qty?parseFloat((i.qty*multi).toFixed(2)):null, cal:Math.round(i.cal*multi), prot:Math.round(i.prot*multi)}}; }}); recipe=JSON.stringify(p); }} addItem('{safe_name}', Math.round({f['calories']}*multi), Math.round({f['protein']}*multi), q, '{f['unit'] or "qty"}'); }} }}" class="sug-item"><b>+ {f["food_name"]}</b><br><span style="color:#8e8e93; font-weight:normal;">{f["qty"] or 1}{f["unit"] or "qty"} | {f["calories"]} kcal</span></div>"""
    ...
    <!DOCTYPE html><html lang="en"><head><meta name="viewport" content="width=device-width, initial-scale=1.0">{CSS}</head><body>
        ...
        <form method="POST" style="margin-top:30px;">
            <div style="margin-bottom:15px;">
                <span class="input-label">Meal Name</span>
            ...
            <label class="fav-toggle" id="meal_fav_label" style="margin-bottom:15px;">
                ...
                <span>Save Meal to Library?</span>
            </label>
            <button type="submit" class="btn-main" style="background:#30d158; color:#000; font-size:1.1rem; padding:18px;">✅ CONFIRM MEAL</button>
        </form>
