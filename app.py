...
def home():
    ...
    goal_c = conn.execute("SELECT value FROM settings WHERE key='daily_goal'").fetchone()['value']
    goal_p = conn.execute("SELECT value FROM settings WHERE key='protein_goal'").fetchone()['value']
    
    total_c = sum(log['calories'] for log in logs)
    total_p = sum(log['protein'] for log in logs)
    conn.close()

    # Lógica do Bulking: Fica VERDE quando passas da meta
    color_c = "#30d158" if total_c >= int(goal_c) else "#fff"
    color_p = "#30d158" if total_p >= int(goal_p) else "#fff"
    ...
    <!DOCTYPE html><html lang="pt"><head><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">{CSS}</head><body>
        <div class="card" style="background: linear-gradient(145deg, #1c1c1e, #000); border: none;">
            ...
            <h1 style="font-size: 3rem; margin: 5px 0; color: {color_c}; transition: color 0.3s;">
                {total_c} <span style="font-size: 1.2rem; color: #8e8e93; font-weight: normal;">/ {goal_c} kcal</span>
            </h1>
            <p style="color: {color_p}; font-weight: bold; font-size: 1.1rem; margin: 0; transition: color 0.3s;">
        ...
        <div class="card">
            ...
            <div class="sug-container" style="margin-bottom: 15px;">
                {"".join([f'<a href="/quick_add/{f["food_name"]}/{f["calories"]}/{f["protein"]}" class="sug-item"><b>{f["food_name"]}</b><br><span style="color:#8e8e93;">{f["calories"]}k</span></a>' for f in favs]) or '<p style="color:#444; font-size:0.8rem; margin-left:10px;">Sem favoritos.</p>'}
            </div>
            <form method="POST">
...
def build_meal():
    ...
    <!DOCTYPE html><html lang="pt"><head><meta name="viewport" content="width=device-width, initial-scale=1.0">{CSS}</head><body>
        ...
        <div class="sug-container" style="margin-bottom:20px;">
            {"".join([f'<div onclick="addItem(\'{f["food_name"]}\', {f["calories"]}, {f["protein"]})" class="sug-item"><b>+ {f["food_name"]}</b></div>' for f in favs]) or '<p style="color:#444; font-size:0.8rem; margin-left:10px;">Sem favoritos.</p>'}
        </div>
        ...
        <form method="POST" style="margin-top:30px;">
            ...
            <label class="fav-toggle" id="meal_fav_label">
                ...
                <span>Guardar Refeição Inteira na Biblioteca?</span>
            </label>
        ...
        <script>
            ...
            function addItem(name, cal, prot) {{
                totalCal += parseInt(cal); totalProt += parseInt(prot);
                items.push(name); updateUI();
            }}
            function addCustom() {{
...
def history():
    ...
    html_content = "".join([f'<div class="log-item"><div style="text-align:left;"><b style="color:#0a84ff;">{datetime.strptime(d["date"], "%Y-%m-%d").strftime("%d %b")}</b></div><div style="font-weight:bold;">{d["total_c"]} <span style="font-size:0.8rem; color:#8e8e93;">/ {goal_c}k</span> | {d["total_p"]}g P</div></div>' for d in days])
    return f'<!DOCTYPE html><html lang="pt"><head><meta name="viewport" content="width=device-width, initial-scale=1.0">{CSS}</head><body><h2 style="color:#8e8e93; margin-bottom:30px;">DIAS PASSADOS</h2>{html_content or "<p style=\'color:#444;\'>Sem histórico.</p>"}<div class="nav-bar"><a href="/" class="nav-item"><span>🏠</span><br>HOJE</a><a href="/history" class="nav-item active"><span>📅</span><br>HISTÓRICO</a><a href="/manage_favs" class="nav-item"><span>⚙️</span><br>DEFINIÇÕES</a></div></body></html>'
...
def manage_favs():
    ...
    html_favs = "".join([f'<div class="log-item"><div style="text-align:left;"><b>{f["food_name"]}</b><br><small style="color:#8e8e93;">{f["calories"]}k | {f["protein"]}g</small></div><div><a href="/edit_fav/{f["id"]}" style="color:#0a84ff; text-decoration:none; font-weight:bold; margin-right:15px;">EDITAR</a><a href="/delete_fav/{f["id"]}" style="color:#ff453a; text-decoration:none; font-weight:bold;">✕</a></div></div>' for f in favs])
    ...
    <!DOCTYPE html><html lang="pt"><head><meta name="viewport" content="width=device-width, initial-scale=1.0">{CSS}</head><body>
        ...
        <div class="nav-bar"><a href="/" class="nav-item"><span>🏠</span><br>HOJE</a><a href="/history" class="nav-item"><span>📅</span><br>HISTÓRICO</a><a href="/manage_favs" class="nav-item active"><span>⚙️</span><br>DEFINIÇÕES</a></div>
    </body></html>
    """
...
def delete_fav(fav_id):
    conn = get_db_connection(); conn.execute('DELETE FROM favorites WHERE id = ?', (fav_id,)); conn.commit(); conn.close()
    return redirect(url_for('manage_favs'))
