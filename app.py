...
def home():
    ...
    # 4 Mais usados + 2 Mais Recentes (Sem duplicados)
    ...
    for f in display_favs:
        ...
        html_favs += f"""
        <div class="sug-item" onclick="quickAddPrompt(this)" data-name="{f['food_name']}" data-qty="{f['qty'] or 1}" data-unit="{f['unit'] or 'qty'}" data-cal="{f['calories']}" data-prot="{f['protein']}" data-recipe="{recipe_safe}">
    ...
    <div class="log-item" style="cursor:pointer;" onclick="window.location.href='/edit_log/{l['id']}'">
        ...
        <button onclick="event.stopPropagation(); window.location.href='/delete/{l['id']}'" style="background:rgba(255,69,58,0.15); border:1px solid #ff453a; color:#ff453a; font-weight:bold; font-size:1.1rem; padding:10px 15px; border-radius:12px; margin-left:10px;">✕</button>
    </div>""" for l in logs])
    ...
    <!DOCTYPE html><html lang="en"><head><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">{CSS}</head><body>
        ...
        <script>
            function saveToLibAjax() {{
                ...
                let fname = fd.get('food_name') || 'Item';
                let qty = fd.get('qty') || 1; let unit = fd.get('unit') || 'qty';
                let cal = fd.get('calories'); let prot = fd.get('protein');
                ...
                fetch('/ajax_save_fav', {{method: 'POST', body: fd}}).then(() => {{
                    ...
                    // Injeta na UI visualmente
                    let newDiv = document.createElement('div');
                    newDiv.className = 'sug-item';
                    newDiv.innerHTML = `<b>${{fname}}</b><br><span style="color:#8e8e93; font-weight:normal; display:block; margin-top:8px;">${{qty}} ${{unit}} | ${{cal}} kcal</span>`;
                    newDiv.setAttribute('data-name', fname); newDiv.setAttribute('data-qty', qty); newDiv.setAttribute('data-unit', unit);
                    newDiv.setAttribute('data-cal', cal); newDiv.setAttribute('data-prot', prot); newDiv.setAttribute('data-recipe', '');
                    newDiv.onclick = function() {{ quickAddPrompt(this); }};
                    
                    let cont = document.getElementById('quick_add_container');
                    cont.insertBefore(newDiv, cont.firstChild);
                    
                    setTimeout(() => {{ 
                        btn.innerText = "💾 TO LIBRARY"; btn.style.background = "#ff9f0a"; btn.style.color = "#000"; 
...
def library():
    ...
    for f in favs:
        ...
        <div class="log-item fav-search-item" style="cursor:pointer; display:flex;" onclick="window.location.href='/edit_fav/{f['id']}'" data-name="{f['food_name'].lower()}">
            ...
            <button style="background:#0a84ff; color:#fff; font-weight:bold; font-size:1.5rem; padding:5px 15px; border-radius:12px; margin-left:10px; border:none; cursor:pointer;" onclick="event.stopPropagation(); quickAddPrompt(this)" data-name="{f['food_name']}" data-qty="{f['qty'] or 1}" data-unit="{f['unit'] or 'qty'}" data-cal="{f['calories']}" data-prot="{f['protein']}" data-recipe="{recipe_safe}">+</button>
        </div>"""
